...
fun main() = runBlocking {
    ...
    println("[${"%.1f".format(totalTime / 1000.0)}s] 全部完成，总耗时约 ${totalTime / 1000.0} 秒")
}
...
fun main() = runBlocking {
    ...
    println("[${"%.1f".format(totalTime / 1000.0)}s] 加载完成，总耗时约 ${totalTime / 1000.0} 秒")
}
...
suspend fun planTrip(destination: String): String = coroutineScope {
    ...
    val flightInfo = flightDeferred.await()
    val hotelInfo = hotelDeferred.await()
    val weatherInfo = weatherDeferred.await()
    ...
    coroutineScope {
        ...
        val riskDeferred = async {
            try {
                ...
                println("[0.3s] 风控检查异常：${e.message}，已降级处理")
                "风控未通过（已降级处理）"
            }
        }
...
suspend fun performSearch(query: String) {
    delay(2000)
    println("[2.5s] 关于「$query」的搜索结果：42 条")
}
...
suspend fun loadImage() {
    withContext(Dispatchers.IO) {
        delay(500)
        println("${Thread.currentThread().name} - 图片已加载（IO 线程）")
    }
}
...
suspend fun processImage() {
    withContext(Dispatchers.Default) {
        delay(1000)
        println("${Thread.currentThread().name} - 图片处理完成（Default 线程）")
    }
}

suspend fun saveImage() {
    delay(300)
    println("${Thread.currentThread().name} - 图片已保存（回到主线程）")
}

fun main() {
    runBlocking {
        println("${Thread.currentThread().name} - 开始图片处理")

        launch {
            println("${Thread.currentThread().name} - launch 启动")
            loadImage()
            processImage()
            saveImage()
            println("${Thread.currentThread().name} - 全部处理完成")
        }
    }
}
