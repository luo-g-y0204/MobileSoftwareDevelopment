...
suspend fun windowA() {
    delay(1200)
    println("[1.2s] A窗口：包裹已出库")
}

suspend fun windowB() {
    delay(900)
    println("[0.9s] B窗口：包裹已出库")
}

suspend fun windowC() {
    delay(1500)
    println("[1.5s] C窗口：包裹已出库")
}

fun main() = runBlocking {
    println("[0.0s] 快递站开门")

    val jobA = launch { windowA() }
    val jobB = launch { windowB() }
    val jobC = launch { windowC() }

    println("[0.0s] 所有窗口已开放，请等待取件...")

    jobA.join()
    jobB.join()
    jobC.join()

    println("[1.5s] 全部完成，总耗时约 1.5 秒")
}
...
fun main() = runBlocking {
    ...
    val totalTime = measureTimeMillis {
        // 并发调用
        val nickDeferred = async { fetchNickname() }
        val unreadDeferred = async { fetchUnreadCount() }
        val feedDeferred = async { fetchLatestFeed() }

        // 等待结果
        val nickname = nickDeferred.await()
        val unreadCount = unreadDeferred.await()
        val latestFeed = feedDeferred.await()

        // 按格式输出
        println("[1.0s] ========== 用户主页 ==========")
        println("[1.0s] 昵称：$nickname")
        println("[1.0s] 未读消息：$unreadCount 条")
        println("[1.0s] 最新动态：$latestFeed")
    }

    println("[1.0s] 加载完成，总耗时约 1.0 秒")
}
...
fun main() = runBlocking {
    ...
    val riskDeferred = async {
        try {
            ...
            println("[0.3s] 风控检查异常：风控系统异常，已降级处理")
            "风控未通过（已降级处理）"
        }
    }
    ...
    println("[0.5s] ========== 支付结果 ==========")
    println("[0.5s] 余额：$balanceResult")
    println("[0.5s] 风控：$riskResult")
    println("[0.5s] 支付已提交（风控降级模式）")
}
...
fun main() = runBlocking {
    ...
    delay(500)
    searchJob1.cancel()
    println("[0.5s] 搜索「Kotlin」已取消（用户继续输入）")

    // 立即启动第二个搜索协程
    val searchJob2 = launch {
        println("[0.5s] 开始搜索：Kotlin 协程")
...
suspend fun loadImage() = withContext(Dispatchers.IO) {
    delay(500)
...
fun main() = runBlocking {
    ...
    println("[0.0s] $mainThreadName @coroutine#1 - 开始图片处理")

    launch {
        val coroutineThreadName = Thread.currentThread().name
        println("[0.0s] $coroutineThreadName @coroutine#2 - launch 启动")

        loadImage()
        processImage()
        saveImage()

        val finalThreadName = Thread.currentThread().name
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] $finalThreadName @coroutine#2 - 全部处理完成")
    }.join()
}
