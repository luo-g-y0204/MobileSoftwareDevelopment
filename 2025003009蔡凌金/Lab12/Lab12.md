...
fun main() = runBlocking {
   ...
   println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 全部完成，总耗时约 ${"%.1f".format(total / 1000.0)} 秒")
}
...
fun main() = runBlocking {
   ...
   println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 加载完成，总耗时约 ${"%.1f".format(total / 1000.0)} 秒")
}
...
suspend fun planTrip(destination: String) = coroutineScope {
   ...
   val hotels = async { searchHotels(destination) }
   val flights = async { searchFlights(destination) }
...
fun main() = runBlocking {
   ...
   val (f, h, w) = planTrip("上海")
   println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] ========== 旅行计划 ==========")
   println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 机票：$f")
   println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 酒店：$h")
   println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 天气：$w")
}
...
suspend fun checkRiskControlRaw(): String {
   delay(300)
