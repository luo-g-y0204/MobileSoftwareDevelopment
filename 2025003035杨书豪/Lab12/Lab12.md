...
fun log(message: String) {
    val time = String.format("%.1f", (System.currentTimeMillis() - startTime) / 1000.0)
    println("[${time}s] $message")
}
...
fun main() {
    ...
    log("全部完成，总耗时约 ${String.format("%.1f", totalTime / 1000.0)} 秒")
}
```
...
fun main() {
    ...
    log("加载完成，总耗时约 ${String.format("%.1f", totalTime / 1000.0)} 秒")
}
```
...
fun main() {
    runBlocking {
        log("正在为你规划上海之旅...")
        val result = planTrip("上海")
        result.lines().forEach { log(it) }
    }
}
...
fun log(message: String) {
    ...
    println("[${time}s] $threadName - $message")
}
