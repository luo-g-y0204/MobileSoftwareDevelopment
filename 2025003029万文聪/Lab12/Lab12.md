...
fun timePrefix(): String = "[${"%.1f".format((System.currentTimeMillis() - startTime) / 1000.0)}s]"
fun log(message: String) = println("${timePrefix()} $message")
...
fun main() {
    ...
    log("全部完成，总耗时约 ${"%.1f".format(totalTime / 1000.0)} 秒")
}
```
...
fun main() {
    val totalTime = measureTimeMillis {
        runBlocking {
            ...
            log("========== 用户主页 ==========")
            log("昵称：$nickname")
            log("未读消息：$unreadCount 条")
            log("最新动态：$latestFeed")
        }
    }

    log("加载完成，总耗时约 ${"%.1f".format(totalTime / 1000.0)} 秒")
}
```
...
suspend fun planTrip(destination: String): String = coroutineScope {
    ...
    """
    ${timePrefix()} ========== 旅行计划 ==========
    ${timePrefix()} 机票：$flight
    ${timePrefix()} 酒店：$hotel
    ${timePrefix()} 天气：$weather
    """.trimIndent()
}

fun main() = runBlocking {
    val destination = "上海"
    log("正在为你规划${destination}之旅...")
    println(planTrip(destination))
}
```
...
fun main() = runBlocking {
    ...
    delay(500)
    searchJob1.cancelAndJoin()
    log("搜索「Kotlin」已取消（用户继续输入）")
...
fun timePrefix(): String = "[${"%.1f".format((System.currentTimeMillis() - startTime) / 1000.0)}s]"
fun logWithThread(message: String) = println("${timePrefix()} ${Thread.currentThread().name} - $message")

suspend fun loadImage() = withContext(Dispatchers.IO) {
    delay(500)
    logWithThread("图片已加载（IO 线程）")
}

suspend fun processImage() = withContext(Dispatchers.Default) {
    delay(1000)
    logWithThread("图片处理完成（Default 线程）")
}

suspend fun saveImage() {
    delay(300)
    logWithThread("图片已保存（回到主线程）")
}
...
fun main() {
    ...
    runBlocking {
        logWithThread("开始图片处理")

        val job = launch {
            logWithThread("launch 启动")
            loadImage()
            processImage()
            saveImage()
            logWithThread("全部处理完成")
        }
