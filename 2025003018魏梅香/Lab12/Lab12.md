...
fun log(message: String) {
    ...
    println("[${"%.1f".format(elapsedSeconds)}s] $message")
}
...
fun main() = runBlocking {
    log("正在为你规划上海之旅...")
    val planLines = planTrip("上海")
    planLines.forEach { log(it) }
}
...
fun main() = runBlocking {
    ...
    log("开始搜索: Kotlin")
    searchJob = launch { performSearch("Kotlin") }
    delay(500)
    searchJob.cancel()
    log("搜索「Kotlin」已取消（用户继续输入）")
    log("开始搜索: Kotlin 协程")
    searchJob = launch { performSearch("Kotlin 协程") }
    searchJob.join()
}
...
fun log(message: String) {
    ...
    println("[${"%.1f".format(elapsedSeconds)}s] $threadName - $message")
}
