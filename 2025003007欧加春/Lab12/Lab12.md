...
fun main() {
    val totalTime = measureTimeMillis {
        runBlocking {
            ...
            val nickname = async { fetchNickname() }
            val unreadCount = async { fetchUnreadCount() }
            val latestFeed = async { fetchLatestFeed() }
            val n = nickname.await()
            val u = unreadCount.await()
            val l = latestFeed.await()
            println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] ========== 用户主页 ==========")
            println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 昵称：$n")
            println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 未读消息：$u 条")
            println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 最新动态：$l")
        }
    }
...
suspend fun planTrip(destination: String) = coroutineScope {
    ...
    val h = hotel.await()
    val w = weather.await()
    "[${(System.currentTimeMillis() - startTime) / 1000.0}s] 机票：$f\n[${(System.currentTimeMillis() - startTime) / 1000.0}s] 酒店：$h\n[${(System.currentTimeMillis() - startTime) / 1000.0}s] 天气：$w"
}
...
fun main() {
    runBlocking {
        ...
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 余额：$b")
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 风控：$r")
