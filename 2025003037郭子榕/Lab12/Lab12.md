...
suspend fun cookSoup() {
    delay(800)
    println("[%.1fs] 汤已煮好".format((System.currentTimeMillis() - startTime) / 1000.0))
}

suspend fun cookSteak() {
    delay(1500)
    println("[%.1fs] 牛排煎好".format((System.currentTimeMillis() - startTime) / 1000.0))
}

suspend fun cookSalad() {
    delay(500)
    println("[%.1fs] 沙拉已备好".format((System.currentTimeMillis() - startTime) / 1000.0))
}

fun main() {
    println("[%.1fs] 后厨开始工作".format((System.currentTimeMillis() - startTime) / 1000.0))
    ...
    println("[%.1fs] 全部完成，总耗时约 %.1f 秒".format(
...
suspend fun windowA() {
    delay(1200)
    println("[%.1fs] A窗口：包裹已出库".format((System.currentTimeMillis() - startTime) / 1000.0))
}

suspend fun windowB() {
    delay(900)
    println("[%.1fs] B窗口：包裹已出库".format((System.currentTimeMillis() - startTime) / 1000.0))
}

suspend fun windowC() {
    delay(1500)
    println("[%.1fs] C窗口：包裹已出库".format((System.currentTimeMillis() - startTime) / 1000.0))
}

fun main() {
    println("[%.1fs] 快递站开门".format((System.currentTimeMillis() - startTime) / 1000.0))
    val time = measureTimeMillis {
        runBlocking {
            launch { windowA() }
            launch { windowB() }
            launch { windowC() }
            println("[%.1fs] 所有窗口已开放，请等待取件...".format((System.currentTimeMillis() - startTime) / 1000.0))
        }
    }
    println("[%.1fs] 全部完成，总耗时约 %.1f 秒".format(
        (System.currentTimeMillis() - startTime) / 1000.0,
...
fun main() {
    println("[%.1fs] 开始加载用户主页...".format((System.currentTimeMillis() - startTime) / 1000.0))
    val time = measureTimeMillis {
        runBlocking {
            val deferredNickname = async { fetchNickname() }
            val deferredUnread = async { fetchUnreadCount() }
            val deferredFeed = async { fetchLatestFeed() }
            val nickname = deferredNickname.await()
            val unread = deferredUnread.await()
            val feed = deferredFeed.await()
            println("[%.1fs] ========== 用户主页 ==========".format((System.currentTimeMillis() - startTime) / 1000.0))
            println("[%.1fs] 昵称：$nickname".format((System.currentTimeMillis() - startTime) / 1000.0))
            println("[%.1fs] 未读消息：$unread 条".format((System.currentTimeMillis() - startTime) / 1000.0))
            println("[%.1fs] 最新动态：$feed".format((System.currentTimeMillis() - startTime) / 1000.0))
        }
    }
    println("[%.1fs] 加载完成，总耗时约 %.1f 秒".format(
        (System.currentTimeMillis() - startTime) / 1000.0,
        time / 1000.0
    ))
}
...
suspend fun searchFlights(destination: String): String {
    delay(1200)
    println("[%.1fs] 机票已查询".format((System.currentTimeMillis() - startTime) / 1000.0))
    return "北京 → $destination，¥1280"
}

suspend fun searchHotels(destination: String): String {
    delay(800)
    println("[%.1fs] 酒店已查询".format((System.currentTimeMillis() - startTime) / 1000.0))
    return "$destination 中心酒店，¥450/晚"
}

suspend fun checkWeather(destination: String): String {
    delay(500)
    println("[%.1fs] 天气已查询".format((System.currentTimeMillis() - startTime) / 1000.0))
...
fun main() {
    runBlocking {
        println("[%.1fs] 正在为你规划上海之旅...".format((System.currentTimeMillis() - startTime) / 1000.0))
        val plan = planTrip("上海")
        println("[%.1fs] ========== 旅行计划 ==========".format((System.currentTimeMillis() - startTime) / 1000.0))
        plan.lines().forEach { line ->
            println("[%.1fs] $line".format((System.currentTimeMillis() - startTime) / 1000.0))
        }
    }
}
```
...
suspend fun checkBalance(): String {
    delay(500)
    println("[%.1fs] 余额验证通过".format((System.currentTimeMillis() - startTime) / 1000.0))
    return "余额充足"
}
...
fun main() {
    println("[%.1fs] 开始支付验证...".format((System.currentTimeMillis() - startTime) / 1000.0))
    runBlocking {
        val deferredBalance = async { checkBalance() }
        val deferredRisk = async {
            try {
                checkRiskControl()
            } catch (e: RuntimeException) {
                println("[%.1fs] 风控检查异常：${e.message}，已降级处理".format(
                    (System.currentTimeMillis() - startTime) / 1000.0
                ))
                "风控未通过（已降级处理）"
            }
        ...
        val risk = deferredRisk.await()
        println("[%.1fs] ========== 支付结果 ==========".format((System.currentTimeMillis() - startTime) / 1000.0))
        println("[%.1fs] 余额：$balance".format((System.currentTimeMillis() - startTime) / 1000.0))
        println("[%.1fs] 风控：$risk".format((System.currentTimeMillis() - startTime) / 1000.0))
        println("[%.1fs] 支付已提交（风控降级模式）".format((System.currentTimeMillis() - startTime) / 1000.0))
    }
}
```
...
suspend fun performSearch(query: String) {
    delay(2000)
    println("[%.1fs] 关于「$query」的搜索结果：42 条".format((System.currentTimeMillis() - startTime) / 1000.0))
}

fun main() {
    runBlocking {
        println("[%.1fs] 开始搜索：Kotlin".format((System.currentTimeMillis() - startTime) / 1000.0))
        val searchJob1 = launch {
            performSearch("Kotlin")
        }
        delay(500)
        searchJob1.cancel()
        println("[%.1fs] 搜索「Kotlin」已取消（用户继续输入）".format((System.currentTimeMillis() - startTime) / 1000.0))
        println("[%.1fs] 开始搜索：Kotlin 协程".format((System.currentTimeMillis() - startTime) / 1000.0))
...
suspend fun loadImage() {
    withContext(Dispatchers.IO) {
        delay(500)
        println("[%.1fs] %s - 图片已加载（IO 线程）".format(
            (System.currentTimeMillis() - startTime) / 1000.0,
...
suspend fun processImage() {
    withContext(Dispatchers.Default) {
        delay(1000)
        println("[%.1fs] %s - 图片处理完成（Default 线程）".format(
            (System.currentTimeMillis() - startTime) / 1000.0,
            Thread.currentThread().name
        ))
    }
...
suspend fun saveImage() {
    delay(300)
    println("[%.1fs] %s - 图片已保存（回到主线程）".format(
        (System.currentTimeMillis() - startTime) / 1000.0,
        Thread.currentThread().name
    ))
}
...
fun main() {
    println("[%.1fs] %s - 开始图片处理".format(
        (System.currentTimeMillis() - startTime) / 1000.0,
        Thread.currentThread().name
    ))
    runBlocking {
        launch {
            println("[%.1fs] %s - launch 启动".format(
                (System.currentTimeMillis() - startTime) / 1000.0,
                Thread.currentThread().name
            ))
            loadImage()
