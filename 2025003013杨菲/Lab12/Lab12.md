...
fun main() = runBlocking {
    ...
    delay(1200) // 等待所有查询完成
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s]$plan")
}
...
fun main() = runBlocking {
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] ${Thread.currentThread().name} @coroutine#1 - 开始图片处理")
    
    val job = launch {
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] ${Thread.currentThread().name} @coroutine#2 - launch 启动")
        loadImage()
        processImage()
        saveImage()
