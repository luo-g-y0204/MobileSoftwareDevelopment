...
fun main() = runBlocking {
    ...
    val totalTime = measureTimeMillis {
        ...
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 昵称：${nickname.await()}")
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 未读消息：${unreadCount.await()} 条")
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 最新动态：${latestFeed.await()}")
    }
...
suspend fun performSearch(query: String) {
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 开始搜索：$query")
    delay(2000)
