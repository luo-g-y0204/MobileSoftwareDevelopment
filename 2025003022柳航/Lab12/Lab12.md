...
fun main() = runBlocking {
    ...
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 全部完成，总耗时约 ${time / 1000.0} 秒")
}
...
fun main() = runBlocking {
    ...
    val time = measureTimeMillis {
        val name = async { fetchNickname() }
        val count = async { fetchUnreadCount() }
        val feed = async { fetchLatestFeed() }
        
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] ========== 用户主页 ==========")
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 昵称：${name.await()}")
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 未读消息：${count.await()} 条")
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 最新动态：${feed.await()}")
    }
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 加载完成，总耗时约 ${time / 1000.0} 秒")
}
---
...
suspend fun planTrip(destination: String) = coroutineScope {
    ...
    Triple(flight.await(), hotel.await(), weather.await())
}

fun main() = runBlocking {
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 正在为你规划上海之旅...")
    val (flight, hotel, weather) = planTrip("上海")
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] ========== 旅行计划 ==========")
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 机票：$flight")
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 酒店：$hotel")
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 天气：$weather")
}
---
...
fun main() = runBlocking {
    ...
    val bResult = balance.await()
    val rResult = risk.await()
    
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 余额验证通过")
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] ========== 支付结果 ==========")
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 余额：$bResult")
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 风控：$rResult")
