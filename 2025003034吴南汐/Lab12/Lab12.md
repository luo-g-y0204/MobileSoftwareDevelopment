...
suspend fun cookSoup() {
    delay(800)
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 汤已煮好")
}

suspend fun cookSteak() {
    delay(1500)
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 牛排煎好")
}

suspend fun cookSalad() {
    delay(500)
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 沙拉已备好")
}

fun main() = runBlocking {
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 后厨开始工作")
    val totalTime = measureTimeMillis {
        cookSoup()
        cookSteak()
        cookSalad()
    }
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 全部完成，总耗时约 ${totalTime / 1000.0} 秒")
}
...
fun main() = runBlocking {
    ...
    val correctTotalTime = measureTimeMillis {
        val job1 = launch { windowA() }
        val job2 = launch { windowB() }
        val job3 = launch { windowC() }
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 所有窗口已开放，请等待取件...")
        job1.join()
        job2.join()
        job3.join()
    }
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 全部完成，总耗时约 ${correctTotalTime / 1000.0} 秒")
}
...
fun main() = runBlocking {
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 开始加载用户主页...")
    val totalTime = measureTimeMillis {
        // 并发启动三个异步任务
        ...
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] ========== 用户主页 ==========")
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 昵称：$nickname")
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 未读消息：$unreadCount 条")
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 最新动态：$latestFeed")
    }
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 加载完成，总耗时约 ${totalTime / 1000.0} 秒")
}
...
suspend fun planTrip(destination: String): String = coroutineScope {
    // 并发执行三个子任务
    val flightDeferred = async { searchFlights(destination) }
    val hotelDeferred = async { searchHotels(destination) }
    val weatherDeferred = async { checkWeather(destination) }
    
    // 拼接结果
    val flight = flightDeferred.await()
    val hotel = hotelDeferred.await()
    val weather = weatherDeferred.await()
    
    return@coroutineScope "========== 旅行计划 ==========\n机票：$flight\n酒店：$hotel\n天气：$weather"
}

fun main() = runBlocking {
    val destination = "上海"
    
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 正在为你规划${destination}之旅...")
    val tripPlan = planTrip(destination)
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] $tripPlan")
}
...
fun main() = runBlocking {
    ...
    val totalTime = measureTimeMillis {
        ...
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 余额：$balanceResult")
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] 风控：$riskResult")
...
```kotlin
// 请将完整代码粘贴至此（包含所有 Thread.currentThread().name 打印）
import kotlinx.coroutines.*
import kotlin.system.measureTimeMillis

val startTime = System.currentTimeMillis()

suspend fun loadImage(): String = withContext(Dispatchers.IO) {
    delay(500)
    val threadName = Thread.currentThread().name
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] $threadName - 图片已加载（IO 线程）")
    "图片加载完成"
}

suspend fun processImage(): String = withContext(Dispatchers.Default) {
    delay(1000)
    val threadName = Thread.currentThread().name
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] $threadName - 图片处理完成（Default 线程）")
    "图片处理完成"
}
...
suspend fun saveImage(): String {
    delay(300)
    val threadName = Thread.currentThread().name
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] $threadName - 图片已保存（回到主线程）")
    return "图片保存完成"
}

fun main() = runBlocking {
    val mainThread = Thread.currentThread().name
    println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] $mainThread @coroutine#1 - 开始图片处理")
    
    val job = launch {
        val coroutineThread = Thread.currentThread().name
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] $coroutineThread @coroutine#2 - launch 启动")
        
        loadImage()
        processImage()
        saveImage()
        
        println("[${(System.currentTimeMillis() - startTime) / 1000.0}s] ${Thread.currentThread().name} @coroutine#2 - 全部处理完成")
    }
