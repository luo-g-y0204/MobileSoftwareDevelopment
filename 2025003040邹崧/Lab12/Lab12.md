...
&#x20;  - `fetchNickname()` — delay 600ms，返回 `"User\_888"`
...
&#x20;       println("\[${(System.currentTimeMillis() - startTime) / 1000.0}s] 未读消息：$unread 条")

&#x20;       println("\[${(System.currentTimeMillis() - startTime) / 1000.0}s] 最新动态：$feed")
...
&#x20;       appendLine("机票：$flights")

&#x20;       appendLine("酒店：$hotels")

&#x20;       appendLine("天气：$weather")
...
&#x20;   println("\[${(System.currentTimeMillis() - startTime) / 1000.0}s] 余额：$balance")

&#x20;   println("\[${(System.currentTimeMillis() - startTime) / 1000.0}s] 风控：$risk")
...
&#x20;   val searchJob1 = launch { performSearch("Kotlin") }
...
&#x20;   val searchJob2 = launch { performSearch("Kotlin 协程") }
...
fun CoroutineContext.printLog(message: String) {

&#x20;   val threadName = Thread.currentThread().name

&#x20;   val coroutineName = this\[CoroutineName]?.name ?: "coroutine#1"

&#x20;   println("\[${(System.currentTimeMillis() - startTime) / 1000.0}s] $threadName @$coroutineName - $message")
...
&#x20;       coroutineContext.printLog("图片已加载（IO 线程）")
...
&#x20;       coroutineContext.printLog("图片处理完成（Default 线程）")
...
&#x20;   coroutineContext.printLog("图片已保存（回到主线程）")

}



fun main() = runBlocking(CoroutineName("coroutine#1")) {

&#x20;   coroutineContext.printLog("开始图片处理")

&#x20;   

&#x20;   launch(CoroutineName("coroutine#2")) {

&#x20;       coroutineContext.printLog("launch 启动")
...
&#x20;       coroutineContext.printLog("全部处理完成")
